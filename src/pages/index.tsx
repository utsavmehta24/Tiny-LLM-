...
function useCountOnView({ target }: { target: number }) {
...
export default function HomePage() {
  ...
  const stats = [
    { target: 1200, suffix: '+', label: 'Projects Completed' },
    ...
    <div className="bg-gray-900 text-white">
      ...
      <section className="relative flex flex-col items-center justify-center text-center h-screen bg-gray-900 overflow-hidden">
        ...
        <motion.div
          ...
          className="mt-8"
        >
          <Link href="/dashboard" className="px-8 py-3 bg-teal-500 hover:bg-teal-600 text-white font-semibold rounded-lg shadow-md transform hover:scale-105 transition duration-300">
            Get Started
          </Link>
        </motion.div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, idx) => (
              <motion.div
                ...
                className="p-6 bg-gray-700 rounded-lg shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-500"
              >
          ...
          <div className="overflow-hidden">
            ...
            >
              {loopingPartners.map((partner, idx) => (
                <div
                  key={idx}
                  className="flex-shrink-0 flex flex-col items-center justify-center w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 bg-gray-800 rounded-full"
                >
                  {partner.icon}
                  <span className="text-sm text-white mt-2 hidden sm:block">{partner.name}</span>
                </div>
              ))}
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              >
                ...
                <p className="text-gray-400 mb-4">{news.date}</p>
                <p className="text-gray-300">{news.body}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-center">
        ...
        >
          ...
          >
            <Link
              href="/dashboard"
              className="inline-block px-8 py-4 bg-teal-500 text-white font-semibold rounded-lg shadow-lg hover:shadow-2xl transition duration-300"
            >
              Get Started Free
            </Link>
          </motion.div>
